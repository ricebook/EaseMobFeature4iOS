...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, IDeviceManagerDelegate, UIActionSheetDelegate>
{
...
@property (strong, nonatomic) RBDNavigationBar *customNavBar;
...
- (instancetype)initWithChatter:(NSString *)chatter isGroup:(BOOL)isGroup
{
    ...
    if (self) {
        ...
        _conversation = [[EaseMob sharedInstance].chatManager conversationForChatter:chatter conversationType:_isChatGroup ?eConversationTypeGroupChat : eConversationTypeChat];
...
//    [self.tableView setHeaderHidden:false];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (![RBAUtils isCurrentSystemAtLeastVersion8]) {
...
- (RBDNavigationBar *)customNavBar {

    if (_customNavBar == nil) {
        
        _customNavBar = [[RBDNavigationBar alloc] initWithFrame:CGRectMake(0, 0, [RBAConstantClass ScreenWidth], [RBAConstantClass NavigationBarHeight])];
        ...
        [_customNavBar.titleView RBD_removeAllSubViews];
        [_customNavBar.titleView addSubview:titleLabel];
        [titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(_customNavBar.titleView).with.insets(UIEdgeInsetsMake(0, 49, 9, 40));
        }];
        
        UIButton *btn = [UIButton new];
        [btn addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
        UIImage *normalImage = [UIImage imageNamed:@"nav_icon_back"];
        UIImage *highlightedImage = [UIImage imageNamed:@"nav_icon_back_press"];
        [btn setImage:normalImage forState:UIControlStateNormal];
        [btn setImage:highlightedImage forState:UIControlStateHighlighted];
        btn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
        btn.contentEdgeInsets = UIEdgeInsetsMake(0, 15, 0, 0);
        [_customNavBar.leftNavView RBD_removeAllSubViews];
        [_customNavBar.leftNavView addSubview:btn];
        [btn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(_customNavBar.leftNavView);
        }];
    }
    ...
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (cell == nil) {
                ...
                if (model.isSender) {
                    UIImage *senderDefaultImage = [UIImage imageNamed:@"profile_avatar_icon"];
                    if ([RBAAccountKit sharedManager].isLogin && [RBAAccountKit sharedManager].loginUserInfo.avatarImageUrl.length != 0) {
                        NSURL *url = [[NSURL alloc] initWithString:[RBAAccountKit sharedManager].loginUserInfo.avatarImageUrl];
                        [cell.headImageView sd_setImageWithURL:url placeholderImage:senderDefaultImage];
                        
                    }else {
                        //Default
                        cell.headImageView.image = senderDefaultImage;
                    }
                }
                else{
                    cell.headImageView.image = [UIImage imageNamed:@"chat_enjoy_logo"];
                }
            }
...
{
    if (url) {
        RBDWebViewController *vc = [[RBDWebViewController alloc] initWithUrl:url navTitle:@""];
...
//                        NSURL *url = [NSURL fileURLWithPath:localPath];
                        ...
                        [weakSelf.messageReadManager showBrowserWithImages:@[localPath]];
                        return ;
                    }
                }
...
- (void)didChoosePhoto {
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:@"选择照片" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"立即拍照",@"从相册选择", nil];
...
- (void)loadMoreMessages
{
    ...
    if ([messages count] > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.dataSource.count > currentCount) {
                needAnimation = false;
            }
            [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[self.dataSource count] - currentCount - 1 inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:needAnimation];
        });
