...
- (void)didChoosePhoto {
    if ([self.delegate respondsToSelector:@selector(didChoosePhoto)]) {
        [self.delegate didChoosePhoto];
    }
}
...
- (void)setupSubviews
{
    ...
    //comment_add_pic
    UIButton *imageBtn = [[UIButton alloc] initWithFrame:CGRectMake(kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
    imageBtn.tag = 99;
    [imageBtn setImage:[UIImage imageNamed:@"comment_add_pic"] forState:UIControlStateNormal];
    [imageBtn addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    textViewLeftMargin += CGRectGetMaxX(imageBtn.frame);
    allButtonWidth += CGRectGetMaxX(imageBtn.frame);
    
    
    // 发送
    UIButton *sendBtn = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - kHorizontalPadding - kInputTextViewMinHeight, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
    sendBtn.tag = 100;
    sendBtn.titleLabel.font = [UIFont systemFontOfSize:14];
    [sendBtn setTitle:@"发送" forState:UIControlStateNormal];
    [sendBtn setTitleColor:[UIColor RBA_colorWithHexString:@"2c3038"] forState:UIControlStateNormal];
    [sendBtn setTitleColor:[UIColor colorWithRed:44/255 green:48/255 blue:56/255 alpha:0.5] forState:UIControlStateHighlighted];
    [sendBtn addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    allButtonWidth += CGRectGetWidth(sendBtn.frame) + kHorizontalPadding * 2.5;
...
//    //chatBar_question chatBar_keyboard
...
//    self.inputTextView.contentMode = UIViewContentModeCenter;
    ...
    _inputTextView.layer.borderColor = [UIColor RBA_colorWithHexString:@"cccccc"].CGColor;
    _inputTextView.layer.borderWidth = 0.65f;
    _inputTextView.textColor = [UIColor RBA_colorWithHexString:@"2c3038"];
    _inputTextView.placeHolderTextColor = [UIColor RBA_colorWithHexString:@"bcbdbf"];
...
//    [self.toolbarView addSubview:self.faceButton];
    [self.toolbarView addSubview:imageBtn];
